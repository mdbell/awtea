...
public class ThreadUtils {
	...
	// Optional monitor - can be null
	private volatile ScheduleTaskListener monitor = null;

	public void setMonitor(ScheduleTaskListener listener) {
		monitor = listener;
	}
	...
	public void runAtFixedRate(String name, Runnable runnable, long periodMillis) {
		synchronized (lock) {
			...
			if (monitor != null) {
				monitor.onCreated(task, name, nextRun, periodMillis);
			}
			queue.add(task);
	...
	public void runOnce(String name, Runnable runnable, long delayMillis) {
		synchronized (lock) {
			...
			if (monitor != null) {
				monitor.onCreated(task, name, nextRun, 0);
			}
			queue.add(task);
		...
		try {
			while (schedulerRunning) {
				...
				synchronized (lock) {
					...
					if (monitor != null) {
						monitor.onQueued(task, task.nextRunTime);
					}
					queue.add(task);
					lock.notifyAll();
				}
	...
	public interface ScheduleTaskListener {
		void onCreated(Object task, String name, long nextRunTime, long periodMillis);
		void onWaiting(Object task);
		void onQueued(Object task);
		void onQueued(Object task, long nextRunTime);
		void onRunning(Object task);
		void onCompleted(Object task);
	}
}
