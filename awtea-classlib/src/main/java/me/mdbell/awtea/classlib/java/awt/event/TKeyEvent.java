...
public class TKeyEvent extends TInputEvent {
    ...
    public boolean isControlDown() {
        return (this.modifiers & FLAG_CTRL) != 0;
    }
    ...
    public enum KeyEvent {
        KEY_PRESSED("keydown", TKeyEvent.KEY_PRESSED),
        KEY_RELEASED("keyup", TKeyEvent.KEY_RELEASED),
        KEY_TYPED("keypress", TKeyEvent.KEY_TYPED);

        private final String type;
        private final int id;

        public static KeyEvent fromType(String type) {
            for (KeyEvent event : values()) {
