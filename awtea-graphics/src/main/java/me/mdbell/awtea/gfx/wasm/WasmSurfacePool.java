 ...
 * Surfaces are pooled by (width, height, pixel format) key to enable efficient reuse
 * for text rendering, temporary buffers, and other high-frequency surface creation scenarios.
 */
public class WasmSurfacePool {
    ...
    /**
     * System property to configure maximum surfaces per key (width, height, format).
     * Default: 4
     */
    ...
    // Map from SurfaceKey to a queue of available surfaces (LRU-ordered: head is most recent)
     ...
     * Removes the oldest (LRU) surface from each key's queue if it has more than one surface.
     * 
