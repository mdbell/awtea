...
Deno.test("Context creation and destruction", async () => {
  ...
  assertEquals(associatedSurfaceId, surfaceId, "Context should be associated with the surface");
...
Deno.test("Context cloning creates independent state", async () => {
  ...
  rasterizer.writeCommand(cmdBuffer1, 1, WasmRasterizer.fillRectCommand(0, 0, 10, 10));
  rasterizer.renderCommands(context1, cmdBuffer1, 2);

  // Clone the context
  const context2 = rasterizer.cloneContext(context1);
  assertNotEquals(context2, context1, "Cloned context should have different ID");
  ...
  rasterizer.writeCommand(cmdBuffer2, 1, WasmRasterizer.fillRectCommand(0, 0, 10, 10));
  ...
  assertPixelEquals(pixels[0], blue, "Surface should be blue after context2 render");

  // Render with context1 again - should still use red (independent state)
  const cmdBuffer3 = rasterizer.createCommandBuffer(1);
  rasterizer.writeCommand(cmdBuffer3, 0, WasmRasterizer.fillRectCommand(0, 0, 10, 10));
  ...
  assertPixelEquals(pixelsAfter[0], red, "Context1 should still have red color state");
...
Deno.test("Independent clip rectangles per context", async () => {
  ...
  rasterizer.writeCommand(cmdBuffer1, 0, WasmRasterizer.setClipRectCommand(0, 0, 10, 20));
  const red = WasmRasterizer.makeARGB(255, 255, 0, 0);
  rasterizer.writeCommand(cmdBuffer1, 1, WasmRasterizer.setColorCommand(red));
  rasterizer.writeCommand(cmdBuffer1, 2, WasmRasterizer.fillRectCommand(0, 0, 20, 20));
  ...
  rasterizer.writeCommand(cmdBuffer2, 0, WasmRasterizer.setClipRectCommand(10, 0, 10, 20));
  const blue = WasmRasterizer.makeARGB(255, 0, 0, 255);
  rasterizer.writeCommand(cmdBuffer2, 1, WasmRasterizer.setColorCommand(blue));
  rasterizer.writeCommand(cmdBuffer2, 2, WasmRasterizer.fillRectCommand(0, 0, 20, 20));
...
Deno.test("Reference counting: surface survives until all contexts destroyed", async () => {
  ...
  const pixels = rasterizer.copySurfacePixels(surfaceId);
  assertPixelEquals(pixels[0], green, "Surface should be accessible and render green");
...
Deno.test("Multiple contexts with independent transforms", async () => {
  ...
  const red = WasmRasterizer.makeARGB(255, 255, 0, 0);
  rasterizer.writeCommand(cmdBuffer1, 0, WasmRasterizer.setColorCommand(red));
  rasterizer.writeCommand(cmdBuffer1, 1, WasmRasterizer.fillRectCommand(0, 0, 5, 5));
  rasterizer.renderCommands(context1, cmdBuffer1, 2);

  // Context2: Translate by (10, 10), render at (0, 0) which becomes (10, 10)
  const cmdBuffer2 = rasterizer.createCommandBuffer(3);
  rasterizer.writeCommand(cmdBuffer2, 0, WasmRasterizer.setTransformCommand(1, 0, 10, 0, 1, 10));
  const blue = WasmRasterizer.makeARGB(255, 0, 0, 255);
  rasterizer.writeCommand(cmdBuffer2, 1, WasmRasterizer.setColorCommand(blue));
  rasterizer.writeCommand(cmdBuffer2, 2, WasmRasterizer.fillRectCommand(0, 0, 5, 5));
  rasterizer.renderCommands(context2, cmdBuffer2, 3);

  // Verify rendering positions
  const pixels = rasterizer.copySurfacePixels(surfaceId);
  assertPixelEquals(pixels[0], red, "Top-left should be red (context1)");
  assertPixelEquals(pixels[10 * 20 + 10], blue, "Position (10,10) should be blue (context2 with transform)");
